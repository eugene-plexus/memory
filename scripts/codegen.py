...
`src/eugene_plexus_memory/_generated/`.
...
GENERATED_DIR = REPO_ROOT / "src" / "eugene_plexus_memory" / "_generated"
WORKING_DIR = REPO_ROOT / ".codegen-cache"

SPECS_TARBALL_URL_TEMPLATE = "https://github.com/eugene-plexus/specs/archive/{ref}.tar.gz"

# memory.yaml has no local `components.schemas` block — every type it
# uses is a $ref to components/common.yaml. datamodel-code-generator
# generates models from `components/schemas`, so feeding it memory.yaml
# directly produces nothing. We feed common.yaml instead, which is a
# complete OpenAPI document (with `paths: {}`) hosting the shared
# schemas. The resulting models.py includes a handful of types memory
# doesn't strictly need (NTState, GenerateRequest, etc.) — that's fine,
# they're cheap and keep the generator's job simple.
SPECS_TO_GENERATE = [
    ("openapi/components/common.yaml", "models.py"),
]
